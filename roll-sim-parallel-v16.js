(() => {
  ...
  if (!NativeWorker || !DATA || window.__rollSimParallelV16Installed) return;
  window.__rollSimParallelV16Installed = true;
  ...
  class ParallelRollSimWorker {
    ...
    postMessage(message) {
      ...
      for (let slot = 0; slot < concurrency; slot += 1) {
        const worker = new NativeWorker('./roll-sim-worker-v16.js?rev=20260824-2004');
