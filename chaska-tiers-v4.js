(() => {
  ...
  function normalizeField(input) {
    ...
    // Re-check after applying the requested value. This protects against invalid states
    // caused by manually lowering one branch while another is already in the next tier.
    ...
    if (maxOther > allowedOtherMax) {
      requested = lastValid.get(id) ?? integerValue(id);
    }
  ...
  function clampToEarnedPoints() {
    ...
    syncing = true;
    // If rolls are lowered, refund from the highest allocations first while preserving tier rules.
    while (totalSpent(current) > earned) {
      ...
      for (const { field } of candidates) {
        const floor = minimumForField(field.id, current);
    ...
    // Re-run the calculator after any automatic refund.
  ...
  function fillToCap(id) {
    ...
    const max = Math.min(capForField(id, current), maxByPoints);
  ...
  function refresh() {
    ...
    setText('uvChaskaTier', currentTier.toLocaleString());
    setText('uvChaskaCap', cap.toLocaleString());
    ...
    setText('uvChaskaUnlockText', `Reach ${cap.toLocaleString()} on all four stats to unlock ${(cap + STEP).toLocaleString()}.`);
    setText('uvChaskaUnlockCount', `${completedAtCap} / 4`);

    const fill = $('uvChaskaUnlockFill');
    if (fill) fill.style.width = `${(completedAtCap / 4) * 100}%`;

    // Replace the old compact points value with the same accurate earned/spent information.
    ...
    for (const field of FIELDS) {
      const input = $(field.id);
      if (!input) continue;
      const fieldCap = capForField(field.id, current);
      const fieldFloor = minimumForField(field.id, current);
      const card = input.closest('.uv-chaska-stat-card');
      if (card) {
        card.dataset.cap = String(fieldCap);
        card.classList.toggle('at-cap', current[field.id] >= fieldCap);
        card.classList.toggle('tier-locked', current[field.id] >= fieldCap && Math.min(...Object.entries(current).filter(([key]) => key !== field.id).map(([, value]) => value)) < fieldCap);
      }
      input.max = String(Math.min(fieldCap, current[field.id] + available));
  ...
  function injectStyles() {
    ...
    style.textContent = `
      .uv-chaska-dashboard{display:grid;gap:10px;margin:-2px 0 12px}.uv-chaska-status{display:grid;grid-template-columns:repeat(5,minmax(0,1fr));gap:6px}.uv-chaska-status>div{padding:8px 9px;border:1px solid var(--line);border-radius:9px;background:var(--panel-2)}.uv-chaska-status span{display:block;color:var(--muted);font-size:.55rem;font-weight:850;letter-spacing:.06em;text-transform:uppercase}.uv-chaska-status strong{display:block;margin-top:3px;font-size:.83rem}.uv-chaska-status .available{border-color:color-mix(in srgb,var(--blue) 42%,var(--line))}.uv-chaska-status .available strong{color:var(--blue)}
      ...
      @media(max-width:800px){.uv-chaska-status{grid-template-columns:repeat(3,1fr)}}@media(max-width:520px){.uv-chaska-status{grid-template-columns:repeat(2,1fr)}.uv-chaska-controls{grid-template-columns:38px 38px 1fr}.uv-chaska-controls .refund{grid-column:1/-1}}
    `;
  ...
  function init() {
    ...
    $('resetBtn')?.addEventListener('click', () => setTimeout(refresh, 0));
    refresh();
  }
