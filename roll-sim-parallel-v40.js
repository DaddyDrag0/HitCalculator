(() => {
  ...
  if (!NativeWorker || !DATA || window.__rollSimParallelV40Installed) return;
  window.__rollSimParallelV40Installed = true;
  ...
  class ParallelRollSimWorker {
    ...
    postMessage(message) {
      ...
      const workerUrl = quickMode
        ? './roll-sim-worker-v39.js?rev=20260826-1702'
        : './roll-sim-worker-v38.js?rev=20260826-1702';
      let nextTask = 0;
      let completed = 0;
      let failed = false;
      ...
      for (let slot = 0; slot < concurrency; slot += 1) {
        const worker = new NativeWorker(workerUrl);
