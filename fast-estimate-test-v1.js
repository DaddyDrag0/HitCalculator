(() => {
  ...
  function ensurePanel() {
    ...
    if (!panel) {
      ...
      panel.innerHTML = '<div class="rs-fast-test-empty"><strong>Fast Estimate Test</strong><span>Click Fast Estimate to calculate probability-based results without simulating every roll.</span></div>';
  ...
  function render(result) {
    ...
    const comboRows = [...result.combos]
      ...
      .map((row) => `
      <tr><td>${esc(row.label)}</td><td>${esc(compact(row.expectedPerRun))}</td><td>${esc(pct(row.share))}</td><td>${esc(compact(row.expectedTotal))}</td></tr>`).join('');
    ...
    }).join('') : '<tr><td colspan="6">No outcome reached the test display threshold.</td></tr>';

    panel.innerHTML = `
      <div class="rs-fast-test-head">
        <div><span>TEST ONLY</span><strong>Fast Estimate</strong><small>Probability calculation, not random simulated run histories.</small></div>
      ...
      <section class="rs-fast-test-card">
        <div class="rs-fast-test-title"><strong>Rarity Results</strong><small>Compare these to Normal simulation averages.</small></div>
      ...
      <section class="rs-fast-test-card">
        <div class="rs-fast-test-title"><strong>Border Combinations</strong><small>Expected PCRG combination counts.</small></div>
      ...
      <section class="rs-fast-test-card">
        <div class="rs-fast-test-title"><strong>Likely Highest Outcomes</strong><small>Rarest outcomes with at least a 0.1% chance across all selected runs.</small></div>
  ...
  function runEstimate() {
    ...
    panel.innerHTML = '<div class="rs-fast-test-empty"><strong>Calculating Fast Estimate…</strong><span>This should finish in seconds because it does not loop through every roll.</span></div>';
    const cleanup = () => {
    ...
    worker.addEventListener('message', (event) => {
      ...
      if (data.type === 'estimate-result') {
        render(data);
        cleanup();
      } else if (data.type === 'estimate-error') {
        panel.innerHTML = `<div class="rs-fast-test-empty"><strong>Fast Estimate Error</strong><span>${esc(data.message || 'Unknown error')}</span></div>`;
        cleanup();
      }
    });
  ...
  function moveWeatherAbovePacks() {
    ...
    root.querySelectorAll('.rs-scenario[data-scenario]').forEach((panel) => {
      ...
      for (const node of [title, mode, fixed, structures]) if (node) panel.insertBefore(node, pack);
    });
  }
  ...
  function install() {
    ...
    if (scenarios && scenarios.dataset.fastEstimateObserver !== '1') {
      scenarios.dataset.fastEstimateObserver = '1';
      new MutationObserver(moveWeatherAbovePacks).observe(scenarios, { childList:true, subtree:true });
    }
    return true;
  }
  ...
  style.textContent = `
    ...
    #rsFastEstimateTestResults{display:grid;gap:10px;margin-top:16px}
    #rsFastEstimateTestResults .rs-fast-test-empty{min-height:90px;display:grid;place-content:center;gap:5px;padding:20px;border:1px dashed var(--line-2);border-radius:12px;text-align:center;color:var(--muted)}
    #rsFastEstimateTestResults .rs-fast-test-empty strong{color:var(--text)}
    #rsFastEstimateTestResults .rs-fast-test-head{display:flex;align-items:flex-end;justify-content:space-between;gap:16px;padding:12px 14px;border:1px solid color-mix(in srgb,var(--galaxy,#a78bfa) 45%,var(--line));border-radius:11px;background:var(--panel-2)}
    #rsFastEstimateTestResults .rs-fast-test-head>div:first-child>span{display:block;color:var(--galaxy,#a78bfa);font-size:.54rem;font-weight:950;letter-spacing:.08em}
    #rsFastEstimateTestResults .rs-fast-test-head strong{display:block;font-size:.9rem}.rs-fast-test-head small{display:block;margin-top:3px;color:var(--muted);font-size:.55rem}
    #rsFastEstimateTestResults .rs-fast-test-summary{display:flex;flex-wrap:wrap;justify-content:flex-end;gap:6px}.rs-fast-test-summary b{padding:5px 8px;border:1px solid var(--line);border-radius:7px;background:var(--panel);font-size:.57rem}
    #rsFastEstimateTestResults .rs-fast-test-card{padding:12px;border:1px solid var(--line);border-radius:11px;background:var(--panel-2)}
    #rsFastEstimateTestResults .rs-fast-test-title{display:flex;justify-content:space-between;gap:12px;align-items:baseline;margin-bottom:9px}.rs-fast-test-title strong{font-size:.68rem;text-transform:uppercase;letter-spacing:.05em}.rs-fast-test-title small{color:var(--muted);font-size:.54rem}
    #rsFastEstimateTestResults td strong{display:block}#rsFastEstimateTestResults td small{display:block;margin-top:2px;color:var(--muted);font-size:.52rem}
    @media(max-width:700px){#rsFastEstimateTestResults .rs-fast-test-head{display:block}.rs-fast-test-summary{justify-content:flex-start!important;margin-top:8px}.rs-fast-test-title{display:block!important}.rs-fast-test-title small{display:block;margin-top:3px}}
  `;
