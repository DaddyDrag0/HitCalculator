importScripts('./roll-sim-data-v16.js?rev=20260824-2004');
...
const MASK_MULTIPLIERS = Array.from({ length: MASK_COUNT }, (_, mask) => {
  let value = 1;
  for (let i = 0; i < BN.length; i += 1) if (mask & (1 << i)) value *= DATA.borders[BN[i]].multiplier;
  return value;
});
...
function clampLevel(value, max) { return Math.max(0, Math.min(max, Math.floor(Number(value) || 0))); }
...
function baseBorderMultipliers(build, weather) {
  ...
  for (const name of BN) {
    let normal = (1 + (charm[name] || 0) + skill(build, name)) * all;
    normal *= structureMultiplier(name, clampLevel(build.structures?.[name], 5));
    if (name === 'Galaxy' && weather === 'Eclipse') normal *= 1.15;
    out[name] = Math.max(0, (normal * borderBoost + dungeonBonus(build, name) + chaska(build.chaska?.[name], chaskaRates[name])) * divine);
  }
  return out;
}
...
function adjustedCardRarity(card, build, weather, weatherStructures) {
  if (card.sin && !build.modifiers?.bossPot) return null;
...
function makeCardDistribution(build, weather, surgeActive, diceActive, weatherStructures) {
  ...
  if (sum > 0 && Math.abs(sum - 1) > 1e-12) for (let i = 0; i < probabilities.length; i += 1) probabilities[i] /= sum;
...
function geometric(probability, rng) {
  ...
  return Math.max(1, Math.ceil(Math.log(Math.max(Number.EPSILON, 1 - rng())) / Math.log(1 - probability)));
}
...
function splitDiceRolls(n, enabled, counter) {
  ...
  return { normal: n - dice, dice, counter: afterFirst % 25 };
}
...
function parseGroupKey(key) {
  ...
  return { weather: weatherRaw === 'Normal' ? null : weatherRaw, surge: surgeRaw === '1', dice: diceRaw === '1', vic: Number(vicRaw) };
}
...
function simulateRun(scenario, totalSeconds, seed) {
  ...
  let bestCardIndex = -1, bestMask = 0, bestEffective = -1, bestCount = 0;
  for (const [groupKey, n] of rollState.groups) {
    ...
    for (let roll = 0; roll < n; roll += 1) {
      ...
      if (effective > bestEffective) {
        bestCardIndex = cardIndex; bestMask = mask; bestEffective = effective; bestCount = 1;
      } else if (effective === bestEffective && cardIndex === bestCardIndex && mask === bestMask) bestCount += 1;
    }
  }
  let uniqueCards = 0;
  for (const count of cardTotals) if (count > 0) uniqueCards += 1;
  const cardMasks = Array.from({ length: CARDS.length }, (_, i) => Array.from(cardMasksFlat.subarray(i * MASK_COUNT, i * MASK_COUNT + MASK_COUNT)));
  return {
    seed, totalRolls: rollState.totalRolls, weatherRolls: rollState.weatherRolls, uniqueCards,
    cardTotals: Array.from(cardTotals), cardMasks, borderTotals: Array.from(borderTotals), comboTotals: Array.from(comboTotals),
...
function aggregateRuns(runs) {
  ...
  const cardTotals = new Array(CARDS.length).fill(0), cardHitRuns = new Array(CARDS.length).fill(0);
  const cardMasks = Array.from({ length: CARDS.length }, () => new Array(MASK_COUNT).fill(0));
  const borderTotals = new Array(BN.length).fill(0), borderHitRuns = new Array(BN.length).fill(0);
  const comboTotals = new Array(MASK_COUNT).fill(0), comboHitRuns = new Array(MASK_COUNT).fill(0);
  const weatherRolls = {};
  let totalRolls = 0, uniqueTotal = 0, bestPull = null;
  for (const run of runs) {
    ...
    for (let i = 0; i < CARDS.length; i += 1) {
      const c = run.cardTotals[i]; cardTotals[i] += c; if (c > 0) cardHitRuns[i] += 1;
      for (let mask = 0; mask < MASK_COUNT; mask += 1) cardMasks[i][mask] += run.cardMasks[i][mask];
    }
    for (let i = 0; i < BN.length; i += 1) { borderTotals[i] += run.borderTotals[i]; if (run.borderTotals[i] > 0) borderHitRuns[i] += 1; }
    for (let mask = 0; mask < MASK_COUNT; mask += 1) { comboTotals[mask] += run.comboTotals[mask]; if (run.comboTotals[mask] > 0) comboHitRuns[mask] += 1; }
    if (run.bestPull && (!bestPull || run.bestPull.effectiveRarity > bestPull.effectiveRarity)) bestPull = { ...run.bestPull };
  }
  return { runs: runs.length, totalRolls, averageRolls: totalRolls / count, averageUniqueCards: uniqueTotal / count, cardTotals, cardHitRuns, cardMasks, borderTotals, borderHitRuns, comboTotals, comboHitRuns, weatherRolls, bestPull };
}
function randomSeed() { return (Math.floor(Math.random() * 0xFFFFFFFF) ^ Date.now()) >>> 0; }
  ...
  try {
    ...
    for (let s = 0; s < scenarios.length; s += 1) {
      ...
      for (let r = 0; r < runCount; r += 1) {
        runs.push(simulateRun(scenarios[s], totalSeconds, randomSeed()));
        completed += 1;
